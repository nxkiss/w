...
namespace memory {
	PVOID get_system_module_base(const char* module_name)
	{
		...
		PRTL_PROCESS_MODULES modules = (PRTL_PROCESS_MODULES)ExAllocatePoolWithTag(NonPagedPool, bytes, 0x50794C73);
	...
	ULONG get_module_base_x64(PEPROCESS proc)
	{
		return (ULONG64)PsGetProcessSectionBaseAddress(proc);
	}

	ULONG get_module_base_x84(PEPROCESS process, LPCWSTR module_name)
	{
		if (!process) { return 0; }
	...
	HANDLE get_process_id(const char* process_name)
	{
		...
		auto buffer = ExAllocatePoolWithTag(NonPagedPool, buffer_size, 'PyLs');

		if (!buffer)
		{
			DbgPrint(0, 0, "failed to allocate, failed to find proc id");
			return 0;
		}
		...
		{
			DbgPrintEx(0, 0, "failed to convert string Pid");
	...
	bool write_to_read_only_memory(void* adress, void* buffer, size_t size) {
		PMDL Mdl = IoAllocateMdl(adress, size, FALSE, FALSE, NULL);
