...
namespace hook {
	NTSTATUS hook_handler(PVOID called_param) {
		...
		if (m->base) {
			...
			if (STATUS_SUCCESS(PsLookupProcessByProcessId(m->pid, &process))) {
				if (m->pid == memory::get_process_id("valorant.exe")) {
					KAPC_STATE apc;
					KeStackAttachProcess(process, &apc);

					LONG b = memory::get_module_base_x84(process, L"client.dll");
			...
			else if (NT_SUCCESS(PsLookupProcessByProcessId(m->pid, &process))) {
			...
			memory::read_kernel_memory(m->pid, m->buffer, (PVOID)m->address, m->size);
		}
	...
	bool call_kernel_function(void* kernel_function_adress) {
		...
		uintptr_t test_address = reinterpret_cast<uintptr_t>(kernel_function_adress);
